...
const CaseProgressIndicator: React.FC<CaseProgressIndicatorProps> = ({step, icon1, icon2, icon3, icon4, icon5, width=350, height=40, progressColor = '#5C68555', backgroundColor = '#E0E0E0'}) => {
  const totalSteps = 4;
  ...
  return(
    <View style={[styles.generalProgressContainer, {width: width, height: height}]}>
      ...
      <View style={[styles.iconContainer, {width}]}>
        ...
        <View style={[styles.iconCaptivity, {left: "85%"}]}>
          <Image source={icon5} style={[styles.icon, {width: iconHeight, height: iconHeight}, step >= 1 && styles.activeIcon]}/>
        </View>
      </View>
    </View>
  );
